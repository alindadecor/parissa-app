...
function Stepper({ step, onBackToHome }: { step: string; onBackToHome: () => void }) {
  return (
    <div className="flex items-center gap-3 sm:gap-[42px] flex-wrap sm:flex-nowrap mb-8 sm:mb-14">
      <div className="text-sm tracking-[0.16em] whitespace-nowrap">{step}</div>
...
function EditorialSidebar() {
  return (
    <aside
      className="relative min-h-[330px] lg:sticky lg:top-0 lg:h-screen overflow-hidden flex flex-col justify-between p-6 sm:p-9 lg:p-[50px] text-[#f8f0e7]"
      style={{
        backgroundColor: '#2C1810',
        backgroundImage:
          'linear-gradient(180deg, rgba(44,24,16,.25), rgba(44,24,16,.65)), url(https://cdn.shopify.com/s/files/1/1011/5058/9226/files/parissa-hero-woman-editorial.webp)',
...
export const Step01PlaceShape: React.FC<Step01Props> = ({
  ...
  const [view, setView] = useState<'place' | 'shape'>('place');
  ...
  return (
    <main className="min-h-[100dvh] w-full overflow-x-hidden bg-[#f7f3ed] text-[#17242c]">
      <div className="grid lg:grid-cols-[31%_69%]">
        <EditorialSidebar />

        <section className="min-w-0 bg-[#f7f3ed] px-4 sm:px-[4.4vw] pt-8 sm:pt-9 pb-10">
          {view === 'place' ? (
            <>
              <Stepper step="01 / 05" onBackToHome={onBackToHome} />

              <section className="min-w-0 max-w-[680px]">
                <div className="text-[13px] tracking-[0.2em] mb-4">01. PLACE</div>
                <h1 className="font-serif-luxury text-[42px] sm:text-5xl lg:text-[66px] leading-[0.9] text-[#433329] mt-0 mb-5 max-w-[600px] font-medium">
                ...
                <button
                  ...
                  className="w-full rounded-full bg-[#6E5A49] text-white py-[18px] text-[15px] cursor-pointer flex items-center justify-center gap-2 hover:bg-[#5f4c3d] transition-colors mt-9"
                >
                  Continue to Shape
                  <ArrowRight size={17} />
                </button>
            ...
            <>
              <Stepper step="01 / 05 · SHAPE" onBackToHome={onBackToHome} />
                  ...
                  {(['round', 'oval', 'marquise'] as DiamondShape[]).map((s) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="p-1.5 px-[10px] pb-4 min-h-[290px]">
                          <div className="relative h-[140px] rounded-[7px] overflow-hidden mb-4">
                            <img
                              src={SHAPE_IMAGES[s]}
                              alt={meta.name}
                              className="w-full h-full object-cover"
